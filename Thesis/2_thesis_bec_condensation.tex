...
These publications led to Bose-Einstein statistics, a model that explains the energetic behavior of a non-interacting gas of indistinguishable bosons. For a total number of particles $N_T$, the normalized mean occupation number of a given non-degenerate state $i$ with energy $E_i$ is:

\begin{equation}\label{eq:bose-einstein_distribution}
	f(E_i) = \frac{1}{e^{(E_i - \mu)/k_B T} - 1}
\end{equation}

When the gas of bosons is in thermal equilibrium at a temperature $T$, Boltzmann constant $k_B$ and the chemical potential $\mu$, which depends of $N_T$ and $T$.
