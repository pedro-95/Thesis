...
\subsection{Transversal cooling}
...
This technique relies on the force applied on particles by red detuned near resonant photons. From Eq. \eqref{eq:rms_velocity}, it can be seen that the temperature of an ideal gas is proportional to its velocity. So this light-produced force, known as radiation pressure, can be used as a way to lower the temperature of an atomic gas simply by reducing its overall speed. To gain an insight into the physical model of this force, it is necessary to analyze a two-level atomic system.

Due to this, we consider an atom with a ground state $\ket{g}$, an excited state $\ket{e}$ and an energy difference between both states of $\Delta E = \hbar \omega_0$, where $\omega_0$ represents the atomic resonance frequency. When the atom is at rest with a state $\ket{g}$, it can absorb a photon with resonant frequency $\omega_0$ and get excited into $\ket{e}$. At this moment, the photon transfers its energy $\hbar \omega_0$ and momentum $\abs{\vec{p}} = \hbar k$, into the atom.
